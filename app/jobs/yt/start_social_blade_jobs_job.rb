class StartSocialBladeJobsJob < ApplicationJob
  ...
  def perform(*args)
    social_blade_urls.each do |url|
      ...
      case url
      when /twitter/
        TwitterSbListJob.perform_async(url)  
      when /engagements/
        TwitterEngagementSbListJob.perform_async(url)
      when /twitch/
        TwitchSbListJob.perform_async(url)
      when /youtube/
        YtSbListJob.perform_async(url)
      when /instagram/
        InstaSbListJob.perform_async(url)
      else
